...
│   ├── ReportController.php         # ✅ PDF report generation (Dompdf + pChart)
...
| `php-generateAdminReport.php` | GET | `/admin/dashboard/report` | `Admin\ReportController::generate` | ✅ | Server-side PDF via Dompdf + pChart charts. Use `POST` (not GET) to send filter params. Button on dashboard. Uses `end_document` callback for page numbering. |
...
├── ChartImageService.php       # ✅ Server-side chart rendering (pChart: pie, doughnut, bar)
└── DeepSeekService.php         # ✅ AI interpretation of assessment results (already existing)
```
...
### 3.4 Admin Dashboard (✅ Complete — PDF Report ✅)

**Components:**
1. **Stat cards** — Total Takers, Today's Submissions, Church Code, Church Name
2. **Charts (Chart.js + chartjs-plugin-datalabels):**
...
6. **PDF Report** — "Generate Report" button next to filters. Uses Dompdf (`barryvdh/laravel-dompdf`) with server-side pChart chart images (`szymach/c-pchart`). Report includes: header (church name, generated by/on), filters applied table, summary stat, 7 chart sections (chart image + data table per chart), detailed user table (Name, Email, Eligible Ministries). Page numbering via Dompdf `end_document` callback + Canvas `text()`. File: `{ChurchName}_Dashboard_Report.pdf`.
...
- **Form Requests** — Auth Form Requests in `app/Http/Requests/Auth/`: `LoginRequest`, `SendVerificationRequest`, `ChangePasswordRequest`. `RegisterRequest` and `CheckEmailRequest` removed (logic merged into `SendVerificationRequest` + `verifyRegistration`). Admin Form Request: `GenerateReportRequest` at `app/Http/Requests/Admin/GenerateReportRequest.php` (validates 11 filter params for PDF report).
...
// Admin panel — middleware('admin') (✅ 16 routes)
Route::prefix('admin')->middleware('admin')->group(function () {
    ...
    POST /dashboard/report                 Admin\ReportController@generate            admin.dashboard.report
...
composer require barryvdh/laravel-dompdf  # ✅ Installed v3.1.2 — PDF generation (Phase 3)
composer require szymach/c-pchart         # ✅ Installed v3.1.1 — Server-side chart images (pChart fork for PHP 8+)
composer require openai-php/laravel       # ❌ Phase 4 — OpenAI integration (using DeepSeek instead)
...
# npm install jspdf jspdf-autotable              # ✅ Not needed — went server-side with Dompdf
...
### Phase 3: Admin Panel (✅ Complete — PDF Report ✅)
...
16. ✅ PDF Report — Dompdf + pChart server-side. ReportController, ChartImageService, dedicated Blade template. "Generate Report" button on dashboard. Filters included in report.
...
12. **Vite entry points:** `admin.js` (sidebar toggle), `auth.js` (login + register pages — togglePassword, toast, forgot modal, auto-show verify), `admin-dashboard.js` (Chart.js dashboard), `assessment.js` (puzzle animation + NEXT button delay), `report-button.js` (Generate Report form submission) — all registered in `vite.config.js`
...
- **Form validation** — All validation in Form Requests (`SendVerificationRequest`, `ChangePasswordRequest`, `LoginRequest`, `GenerateReportRequest`). Password strength rules removed from JS.
- **PDF generation** — Moved from client-side `jspdf` to server-side `Dompdf` + `ChartImageService` (pChart). ReportController generates entire PDF server-side without JS dependencies.
- **Chart images for PDF** — Server-side rendered via pChart (`CpChart\*` namespace). Pie, doughnut, and bar charts generated as PNG images from aggregated UserReport data.
