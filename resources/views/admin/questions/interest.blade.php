...
@section('title', 'Interest & Passion Questions - PERFIT')
    ...
    <div class="card border-0 shadow-sm mt-3" style="border-radius: 16px;">
        <div class="card-body p-4">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start mb-3 gap-2">
                <div>
                    <h5 class="fw-semibold mb-1">Interest & Passion Questions</h5>
                    <p class="text-muted small mb-0">
                        Edit the interest and passion questions to assess volunteer <strong>preferences</strong> for each ministry category.
                        Click on a cell to edit the text directly.
                    </p>
                </div>
                <div class="d-flex gap-2 flex-shrink-0">
                    <button class="noToAdmin btn btn-outline-secondary btn-sm" id="resetInterestBtn">Reset all to default</button>
                    <button class="btn btn-primary btn-sm" id="saveInterestBtn">Save all changes</button>
                </div>
            </div>
            <div style="overflow-x: auto; max-width: 100%; -webkit-overflow-scrolling: touch;">
                <table class="table table-bordered table-hover align-middle mb-0" id="interestQuestions" style="width: 100%; min-width: 650px;">
                    ...
                    <tbody>
                        @forelse($questions as $categoryName => $group)
    ...
    <div class="modalOverlay" id="resetModalOverlay">
        <div class="modalBox">
            <div class="modalBoxHeader">
                <h5 style="margin:0;font-weight:600;">Reset Interest & Passion Questions</h5>
                <button class="modalCloseBtn" id="resetModalCloseBtn">&times;</button>
            </div>
            <div class="modalBoxBody">
                <p>This will replace all interest & passion questions with the default template. Any changes you made will be permanently lost.</p>
                <ul>
                    <li>30 interest & passion questions across 6 ministry categories</li>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('saveInterestBtn')?.addEventListener('click', function () {
        const rows = document.querySelectorAll('#interestQuestions tbody tr[data-id]');
        const questions = [];
        ...
        fetch('{{ route("admin.questions.interest.update") }}', {
            method: 'POST',
    ...
    document.getElementById('resetInterestBtn')?.addEventListener('click', openResetModal);
    ...
    resetModalConfirm?.addEventListener('click', function () {
        ...
        const btn = document.getElementById('resetInterestBtn');
        btn.disabled = true;
        btn.textContent = 'Resetting...';

        fetch('{{ route("admin.questions.interest.reset") }}', {
            method: 'POST',
