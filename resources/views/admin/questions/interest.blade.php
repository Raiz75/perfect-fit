...
@section('content')
    ...
    <div class="admin-glass-card p-4 mt-3">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start mb-3 gap-2">
            <div>
                <h5 class="admin-section-title">Interest & Passion Questions</h5>
                <p class="admin-section-desc mb-0">Edit the interest and passion questions for each ministry category.</p>
            </div>
            <div class="admin-action-bar mb-0">
                <button class="btn btn-outline-perfit noToAdmin" id="resetInterestBtn">Reset</button>
                <button class="btn primary-btn-perfit btn-sm" id="saveInterestBtn">Save changes</button>
            </div>
        </div>
        <div class="admin-glass-table">
            <div style="overflow-x: auto; max-width: 100%;">
                <table class="table align-middle mb-0" id="interestQuestions" style="min-width: 650px;">
                    <thead><tr><th>Category</th><th style="width:140px;">#</th><th>English</th><th>Tagalog</th></tr></thead>
                    <tbody>
                        @forelse($questions as $categoryName => $group)
                            @foreach($group as $question)
                                <tr data-id="{{ $question->id }}">
                                    <td class="fw-medium">{{ $categoryName }}</td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('saveInterestBtn')?.addEventListener('click', function () {
        const rows = document.querySelectorAll('#interestQuestions tbody tr[data-id]'); const qs = [];
        ...
        fetch('{{ route("admin.questions.interest.update") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':csrfToken},body:JSON.stringify({questions:qs})})
        .then(r=>r.json()).then(d=>{notify(d.message,d.success?'success':'danger')}).catch(()=>notify('Error.','danger'))
        .finally(()=>{btn.disabled=false;btn.textContent='Save changes';});
    });
    document.getElementById('resetInterestBtn')?.addEventListener('click',()=>modal.style.display='flex');
    ...
    modal.querySelector('.btn-danger')?.addEventListener('click',function(){
        ...
        fetch('{{ route("admin.questions.interest.reset") }}',{method:'POST',headers:{'X-CSRF-TOKEN':csrfToken}})
