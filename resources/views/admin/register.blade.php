...
<body>
    ...
    <div class="bg-animated">
        ...
        <div class="glass-card p-4 p-md-5 d-flex flex-column align-items-center">

            @php
                $toastMsg = '';
                $toastType = 'error';
                if ($errors->any()) { $toastMsg = $errors->first(); }
                elseif (session('success')) { $toastMsg = session('success'); $toastType = 'success'; }
                elseif (session('error')) { $toastMsg = session('error'); }
            @endphp
            @if ($toastMsg)
